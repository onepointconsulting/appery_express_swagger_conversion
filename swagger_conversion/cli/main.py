...
def swagger_conversion(type: str, location: str, target_folder: str):
    ...
    match type:
        case GenType.TYPE_SWAGGER:
            ...
            swagger_conversion_system = renderer.render_template(
                "swagger_conversion_system.prompt", {}
            )
            ...
            if not target_folder_path.exists():
                ...
                ), f"{target_folder_path} does not exist."

            click.echo(f"{json_path} exists.")
            api = OpenAIClient()
            for i, json_doc in enumerate(parse_document(json_path)):
                res = renderer.render_template(
                    "swagger_conversion.prompt", {"json": json.dumps(json_doc)}
                )
                ...
                try:
                    response = asyncio.run(
                        api.make_request(
                            message, functions=[function_description_swagger_result]
                        )
                    )
